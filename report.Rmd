...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Calibration Plots
calib_data0 <- data.frame(x = predictions_cal[,1], y = ifelse(test_data_PL$Full_time_result_point == 0, 1, 0))
...
calib_data1 <- data.frame(x = predictions_cal[,2], y = ifelse(test_data_PL$Full_time_result_point == 1, 1, 0))
...
calib_data3 <- data.frame(x = predictions_cal[,3], y = ifelse(test_data_PL$Full_time_result_point == 3, 1, 0))
...
calib_data <- data.frame(
  ...
  group = factor(rep(c("Loses", "Draws", "Wins"), each = nrow(predictions_cal)))
)

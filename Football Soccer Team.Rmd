...
=======
>>>>>>> dedd02b34408d8e92de54a129b44a588b6561c7c
```
...
set.seed(907)
bettingBasicPL2018 <- bettingBasicPL2018 %>% 
  mutate(is_train = sample(rep(0:1, length.out = nrow(bettingBasicPL2018))))

#set-up models
library(mgcv)
library(nlme)
init_logit_gam <- gam(as.factor(Full_time_result_point) ~ s(expect_point_H) + s(streak_diff),
                      data = filter(bettingBasicPL2018, is_train == 1),
                      family = binomial, method = "REML")

init_logit_base_gam <- gam(as.factor(Full_time_result_point) ~ s(expect_point_H),
                      data = filter(bettingBasicPL2018, is_train == 1),
                      family = binomial, method = "REML")
=======
bettingBasicPL2018 <- bettingBasicPL2018 %>% 
...
=======
init_logit_gam_point_streak_xg <- gam(list(Full_time_result_point ~ s(expect_point_H)
                      + s(streak_diff) + s(xg_diff), ~ s(expect_point_H)
                      + s(streak_diff) + s(xg_diff)),
>>>>>>> dedd02b34408d8e92de54a129b44a588b6561c7c
                      data = filter(bettingBasicPL2018, is_train == 1),
                      family = binomial, method = "REML")

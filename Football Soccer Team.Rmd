...
test <- test %>%
  mutate(weighted_players_minutes = weighted_minutes * position_weights[match(position, names(position_weights))])
```

```{r}

min_weighted_minutes <- min(test$weighted_players_minutes)
max_weighted_minutes <- max(test$weighted_players_minutes)


position_rankings <- test %>%
  group_by(club_name, position) %>%
  summarize(total_weighted_minutes = sum(weighted_players_minutes))


position_rankings <- position_rankings %>%
  group_by(club_name) %>%
  mutate(normalized_weighted_minutes = (total_weighted_minutes - min_weighted_minutes) /
                                         (max_weighted_minutes - min_weighted_minutes) * 100)


position_rankings <- position_rankings %>%
  group_by(club_name) %>%
  mutate(position_rank = rank(-normalized_weighted_minutes))
...
init_logit_gam <- gam(list(Full_time_result_num ~ s(expect_point_H) + s(streak_diff), 
                      ...
                      data = filter(bettingBasicPL2018, is_train == 1),
                      family = multinom(K = 2))
...
```{r}
#check fot fitness of a model
bettingBasicPL2018 <- bettingBasicPL2018 %>%
  mutate(init_gam_result_prob = as.numeric(predict(init_logit_gam,
                                                   newdata = bettingBasicPL2018,
                                                   type = "response")),
         init_gam_result_class = as.numeric(init_gam_result_prob >= 0.5))
 
bettingBasicPL2018 %>%
  group_by(is_train) %>%
  summarize(correct = mean(init_gam_result_class))
```
 
```{r}
#fitness of a model
bettingBasicPL2018 <- bettingBasicPL2018 %>%
  mutate(init_gam_result_base_prob = as.numeric(predict(init_logit_base_gam,
                                                        newdata = bettingBasicPL2018,
                                                        type = "response")),
         init_gam_result_base_class = as.numeric(init_gam_result_base_prob >= 0.5))
 
bettingBasicPL2018 %>%
  group_by(is_train) %>%
  summarize(correct = mean(init_gam_result_base_class))
```
...
```{r}
#with all
init_logit_gam_point_streak_xg <- gam(as.factor(Full_time_result_point) ~ s(expect_point_H)
                      + s(streak_diff) + s(xg_diff),
                      data = filter(bettingBasicPL2018, is_train == 1),
                      family = binomial, method = "REML")
summary(init_logit_gam_point_streak_xg)
```
...
```{r}
set.seed(234)
train_indices <- sample(nrow(bettingBasicPL2018), nrow(bettingBasicPL2018) * 0.7)
train_data <- bettingBasicPL2018[train_indices, ]
test_data <- bettingBasicPL2018[-train_indices, ]

expect_model <- nnet::multinom(as.factor(Full_time_result_point) ~ expect_point_H, data = train_data)
summary(expect_model)

predictions <- predict(expect_model, newdata = test_data)

library(scoring)
#Predicting the grier score
actual_outcomes <- test_data$Full_time_result_point 
predicted_probs <- predict(expect_model, newdata = test_data, type = "probs")  
...
```{r}
training_data_gams <- bettingBasicPL2018 %>% 
  filter(is_train == 1)
test_data_gams <- bettingBasicPL2018 %>% 
  filter(is_train == 0)

predictions <- predict(init_logit_base_gam, newdata = test_data_gams)

#Predicting the grier score
actual_outcomes_gams <- test_data_gams$Full_time_result_point 
predicted_probs_gams <- predict(init_logit_base_gam, newdata = test_data_gams, type = "response")  
...
```
```{r}
predictions <- predict(init_logit_gam, newdata = test_data_gams)

#Predicting the grier score
actual_outcomes_gams <- test_data_gams$Full_time_result_point 
predicted_probs_gams <- predict(init_logit_gam, newdata = test_data_gams, type = "response")  
cbind(actual_outcomes_gams, predicted_probs_gams)
...
```
```{r}
predictions <- predict(init_logit_gam_xg, newdata = test_data_gams)

#Predicting the grier score
actual_outcomes_gams <- test_data_gams$Full_time_result_point 
predicted_probs_gams <- predict(init_logit_gam_xg, newdata = test_data_gams, type = "response")  
cbind(actual_outcomes_gams, predicted_probs_gams)

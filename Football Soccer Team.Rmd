...
calibration_df <- data.frame(prob = rf_probs, outcome = rf_outcomes)

# Calculate observed probabilities for each bin of predicted probabilities
calibration_curve <- data.frame(obs_prob = c(), pred_prob = c())

for (i in seq(0, 1, by = 0.1)) {
  bin_indices <- calibration_df$prob >= i & calibration_df$prob < i + 0.1
  bin_obs_probs <- mean(calibration_df$outcome[bin_indices])
  calibration_curve <- rbind(calibration_curve, data.frame(obs_prob = bin_obs_probs, pred_prob = i + 0.05))
}

# Plot the calibration curve
plot(calibration_curve$pred_prob, calibration_curve$obs_prob, type = "b",

...
predictions <- predict(expect_model, newdata = test_data_PL,type="probs")
predictions
...
#Histogram
base_data_frame %>% rename(prob_losing = '0', prob_draw = '1', prob_win = '3') %>%
  mutate(row = 1:n()) %>%
  pivot_longer(cols = starts_with('prob')) %>%
  filter(value >= 0.5) %>% 
  ggplot(aes(x = value, fill = name))+
  geom_histogram(bins = 10, position = "dodge") +
  facet_wrap(~ actual_outcomes)
#Dot plot  
base_data_frame %>% rename(prob_losing = '0', prob_draw = '1', prob_win = '3') %>%
  mutate(row = 1:n()) %>%
  pivot_longer(cols = starts_with('prob')) %>%
  ggplot(aes(x = value, y = actual_outcomes, col = name))+
  geom_point() +
  facet_wrap(~ name) +
  geom_smooth() +
  geom_jitter()
...
base_data_frame %>% rename(prob_losing = '0', prob_draw = '1', prob_win = '3') %>%
  mutate(row = 1:n()) %>%
  pivot_longer(cols = starts_with('prob')) %>%
ggplot(aes(x = name, y = value, fill = factor(actual_outcomes))) +
  geom_boxplot() +
  labs(title = "Predicted Probabilities of Match Outcomes",
       x = "Observations",
       y = "Predicted Probability",
       fill = "Actual Outcomes") +
  scale_fill_manual(values = c("#0000FF", "#FFCCCC", "#CCFFE5"),
                    labels = c("Lose", "Draw", "Win")) +
  theme_minimal() +
  scale_x_discrete(labels = c("prob_losing" = "Probability of Losing",
...
```{r}
# Calibration Plots
calib_data0 <- data.frame(x = predictions[,1], y = ifelse(test_data_PL$Full_time_result_point == 0, 1, 0))
plot0 <- ggplot(calib_data0, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
                   title = "Probability Home \nTeam Loses") +
              ylim(0,1)

calib_data1 <- data.frame(x = predictions[,2], y = ifelse(test_data_PL$Full_time_result_point == 1, 1, 0))
plot1 <- ggplot(calib_data1, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
calib_data3 <- data.frame(x = predictions[,3], y = ifelse(test_data_PL$Full_time_result_point == 3, 1, 0))
plot3 <- ggplot(calib_data3, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
# Combine the plots into a a single grid (still three plots)
combined_plot <- plot0 + plot1 + plot3 + plot_layout(ncol = 3) + 
                 ylim(y = y_axis_lim)
...
calib_data <- data.frame(
  x = c(predictions[, 1], predictions[, 2], predictions[, 3]),
...
plot_combined <- ggplot(calib_data, aes(x = x, y = y, color = group)) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(x = "Predicted Probability", y = "Observed fraction",
       title = "Calibration Curves for Different Outcomes") +
  ylim(0, 1) +
...
```{r}
#random forest model with streak and XGs
...
```{r}
predicted_probs <- as.data.frame(predict(init_logit_gam_xg, newdata = test_data_PL, type = "probs"))
...
```{r}
#MSE for GAMs
#mistake in computing dataframe
predicted_probs <- as.data.frame(predict(init_logit_gam, newdata = test_data_PL, type = "probs"))
draw <- base_data_frame %>% 
  filter(actual_outcomes == 1) %>% 
  nrow()
loss <- base_data_frame %>% 
  filter(actual_outcomes == 0) %>% 
  nrow()
win <- base_data_frame %>% 
  filter(actual_outcomes == 3) %>% 
  nrow()

# This is the data frame
gam_streak_data_frame <- base_data_frame%>% 
  rename(prob_losing = '0', prob_draw = '1', prob_win = '3') %>%
  mutate(row = 1:n()) %>%
  mutate(predicted_result = case_when((prob_losing > prob_draw) & (prob_losing > prob_win) ~ 0,
                                      (prob_draw > prob_losing) & (prob_draw > prob_win) ~ 1,
                                      TRUE ~ 2),
         actual_outcomes_num = case_when(actual_outcomes == 3 ~ 2, .default = actual_outcomes),
         residual = (actual_outcomes - predicted_result)) %>% 
  pivot_wider(names_from = actual_outcomes_num, values_from = predicted_result) %>% 
  rename(actual_loss = '0', actual_draw = '1', actual_win = '2') %>%
  mutate(mse_loss = sum((0-actual_loss)^2, na.rm = TRUE)/loss,
         mse_draw = sum((1-actual_draw)^2, na.rm = TRUE)/draw,
         mse_win = sum((2-actual_win)^2, na.rm = TRUE)/win)
```

```{r}
predictions_gam <- predict(init_logit_gam, test_data_PL, type = "response")

predicted_class <- max.col(predictions_gam)

table(predicted_class, test_data_PL$Full_time_result_num)

#Accuracy for multinomial model
predicted_labels_mn <- predict(expect_model, newdata = test_data_PL, type = "class")

true_labels <- test_data_PL$Full_time_result_point

accuracy_multinom <- sum(predicted_labels_mn == true_labels) / length(true_labels)

cat("Accuracy:", accuracy_multinom, "\n")
predicted_labels_gam <- max.col(predicted_probs_gams)

# Calculate accuracy
accuracy_gams_base <- sum(predicted_labels_gam == true_labels) / length(true_labels)

# Print the accuracy
cat("Accuracy base:", accuracy_gams_base, "\n")

```

```{r}
rf_labels <- test_data_PL$Full_time_result_num


rf_predicted_labels <- apply(rf_predictions$predictions, 1, which.max) - 1

#confusion matrix
conf_matrix <- table(Actual = rf_labels, Predicted = rf_predicted_labels)
...
predicted_labels_gamxg <- max.col(predicted_probs)

# Calculate accuracy
accuracy_gamsxg <- sum(predicted_labels_gamxg == true_labels) / length(true_labels)

# Print the accuracy
cat("Accuracy XG:", accuracy_gamsxg, "\n")


```

```{r}
#Accuracy and brier score table

model_names <- c("Random Forest", "Base gams model", "Gams Model xG", "Multinomial")  
accuracies <- c(overall_accuracy, accuracy_gams_base, accuracy_gamsxg, accuracy_multinom)  
brier_scores <- c(0.4418142, 0.25744, 0.2648771, 0.2591487)



model_comparison <- data.frame(Model = model_names, Accuracy = accuracies, Brier_Score = brier_scores)

# Load the required libraries
library(knitr)
library(kableExtra)

# Display the model comparison table using kable
kable_styling(kable(model_comparison, caption = "Model Comparison"), font_size = 14,  bootstrap_options = c("striped", "hover", "condensed")) 

```

```{r}
#multinom with position regression
multinom_pos_reg <- nnet::multinom(as.factor(Full_time_result_point) ~ pos_rating_regression, data = train_data_PL)
summary(multinom_pos_reg)

predictions <- predict(multinom_pos_reg, newdata = test_data_PL)
...
#Predicting the grier score
actual_outcomes_gams <- test_data_PL$Full_time_result_point 
predicted_probs_gams <- predict(init_logit_gam_pos, newdata = test_data_PL, type = "response")  
...
#This is the data frame
gams_dataframe_predict <- gams_dataframe_predict %>% 
  rename(prob_losing = 'X1', prob_draw = 'X2', prob_win = 'X3') %>% 
  pivot_longer(cols = starts_with('prob'))

#Histogram
gams_dataframe_predict %>% 
  filter(value >=0.5) %>% 
  ggplot(aes(x = value, fill = name))+
  geom_histogram(bins = 10, position = "dodge") +
  facet_wrap(~ actual_outcomes_gams)
```

```{r}
#Calibration of Position GAMs
# Calibration Plots
calib_data0_gams <- data.frame(x = predicted_probs_gams[,1], y = ifelse(test_data_PL$Full_time_result_point == 0, 1, 0))
plot0_gams <- ggplot(calib_data0_gams, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
calib_data1_gams <- data.frame(x = predicted_probs_gams[,2], y = ifelse(test_data_PL$Full_time_result_point == 1, 1, 0))
plot1_gams <- ggplot(calib_data1_gams, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
calib_data3_gams <- data.frame(x = predicted_probs_gams[,3], y = ifelse(test_data_PL$Full_time_result_point == 3, 1, 0))
plot3_gams <- ggplot(calib_data3_gams, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
# Combine the plots into a a single grid (still three plots)
combined_plot_gams <- plot0_gams + plot1_gams + plot3_gams + plot_layout(ncol = 3) + 
                 ylim(y = y_axis_lim)

print(combined_plot_gams)

# Calibration Plots into one plot
calib_data_gams <- data.frame(
  x = c(predicted_probs_gams[, 1], predicted_probs_gams[, 2], predicted_probs_gams[, 3]),
  y = c(ifelse(test_data_PL$Full_time_result_point == 0, 1, 0),
        ifelse(test_data_PL$Full_time_result_point == 1, 1, 0),
        ifelse(test_data_PL$Full_time_result_point == 3, 1, 0)),
  group = factor(rep(c("Loses", "Draws", "Wins"), each = nrow(predictions)))
...
```{r}
#multinom with position and betting
...
predictions <- predict(multinom_betting_pos, newdata = test_data_PL)
...
predictions <- predict(multinom_betting_pos_streak, newdata = test_data_PL)
...
predictions <- predict(multinom_full, newdata = test_data_PL)
...
brier_table %>% 
  ggplot(aes(x = type, y = brier.score, fill = type, point)) +
  geom_boxplot() +
...
brier_table %>% 
  ...
  labs(title = "Predicted Probabilities of Match Outcomes",
       ...
       fill = "Actual Outcomes") +
  scale_fill_manual(values = c("#0000FF", "#FFCCCC", "#CCFFE5"),
                    labels = c("multinomial" = "Multinomial Model")) +
  scale_x_discrete(labels = c("multinomial" = "Multinomial Model" ))
  theme_minimal() 
```
...
# Calibration Plots
rand_forest_predictions <- data.frame(rf_probs_base)
calib_data0_rf <- data.frame(x = rand_forest_predictions[,1], y = ifelse(test_data_PL$Full_time_result_point == 0, 1, 0))
plot0_rf <- ggplot(calib_data0_rf, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
calib_data1_rf <- data.frame(x = rand_forest_predictions[,2], y = ifelse(test_data_PL$Full_time_result_point == 1, 1, 0))
plot1_rf <- ggplot(calib_data1_rf, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
calib_data3_rf <- data.frame(x = rand_forest_predictions[,3], y = ifelse(test_data_PL$Full_time_result_point == 3, 1, 0))
plot3_rf <- ggplot(calib_data3_rf, aes(x=x, y=y)) + 
              geom_point() +
              geom_smooth(se = FALSE) +
              geom_abline(slope = 1, intercept=0, linetype="dashed") +
              labs(x="Predicted Probability", y = "Observed fraction",
...
# Combine the plots into a a single grid (still three plots)
combined_plot_rf <- plot0_rf + plot1_rf + plot3_rf + plot_layout(ncol = 3) + 
                 ylim(y = y_axis_lim)

print(combined_plot_rf)

# Calibration Plots into one plot
calib_data_rf <- data.frame(
  x = c(rand_forest_predictions[, 1], rand_forest_predictions[, 2], rand_forest_predictions[, 3]),
  y = c(ifelse(test_data_PL$Full_time_result_point == 0, 1, 0),
        ifelse(test_data_PL$Full_time_result_point == 1, 1, 0),
        ifelse(test_data_PL$Full_time_result_point == 3, 1, 0)),
  group = factor(rep(c("Loses", "Draws", "Wins"), each = nrow(predictions)))
...
```{r}
date_diff = difftime(bettingBasicPL2018$dmy[380], bettingBasicPL2018$dmy[1])/7

bettingBasicPL2018 <- bettingBasicPL2018 %>% 
  mutate(month_num = case_when(((month > 5) & (month < 13)) ~ (month - 7),
                               TRUE ~ (month + 5))) 

#set-up training data (first 2 months as training and October as testing)
train_data_PL <- bettingBasicPL2018 %>% 
  filter(month_num < 2)
...
brier_score['Pos GAMs October'] =  gams_dataframe_predict %>% 
  mutate(actual_outcomes_gams = factor(actual_outcomes_gams)) %>% 
  brier_class(actual_outcomes_gams, X1:X3) %>% 
  select(.estimate)
...
#Brier Score
brier_score['multinomial betting pos October'] = data.frame(
  actual_outcomes,
  predicted_probs
) %>% 
  mutate(actual_outcomes = factor(actual_outcomes)) %>% 
...
```{r}
j <- 2
brier_table_month <- data.frame(type = c("multinomial base", "base GAMs", "streak GAMs", "streak xg GAMs", "rand forest streak Xg", "multinomial pos", "Pos GAMs", "multinomial betting pos", "multinomial betting pos streak", "multinomial ful", "rand forest pos", "Full GAMs", "rand forest base"))

#New set-up training data
for(i in 11:12){
  train_data_month <- bettingBasicPL2018 %>% 
    filter(month %in% 8:(i-1))
  
  test_data_month <- bettingBasicPL2018 %>% 
    filter(month %in% i)
  
  #set-up models
  init_logit_gam <- gam(list(Full_time_result_num ~ s(expect_point_H) + s(streak_diff), 
                           ~s(expect_point_H) + s(streak_diff)),
                      data = filter(train_data_month),
                      family = multinom(K = 2))
  init_logit_base_gam <- gam(list(Full_time_result_num ~ s(expect_point_H), ~s(expect_point_H)),
                           family = multinom(K = 2), 
                           data = train_data_month)
  init_logit_gam_xg <- gam(list(Full_time_result_num ~ s(expect_point_H) + s(streak_diff) 
                              + s(xg_diff), ~s(expect_point_H) + s(streak_diff) 
                              + s(xg_diff)),
                      data = train_data_month,
                      family = multinom(K = 2))
  expect_model <- nnet::multinom(as.factor(Full_time_result_point) ~ expect_point_H, data = train_data_month)
  random_forest <- ranger(Full_time_result_num ~ expect_point_H + streak_diff + xg_diff, data = train_data_month, probability = TRUE)
  multinom_pos_reg <- nnet::multinom(as.factor(Full_time_result_point) ~ pos_rating_regression, data = train_data_month)
  init_logit_gam_pos <- gam(list(Full_time_result_num ~ s(expect_point_H) + s(pos_rating_regression), 
                           ~s(expect_point_H) + s(pos_rating_regression)),
                      data = filter(train_data_month),
                      family = multinom(K = 2))
  multinom_betting_pos <- nnet::multinom(as.factor(Full_time_result_point) ~ pos_rating_regression + expect_point_H, data = train_data_month)
  multinom_betting_pos_streak <- nnet::multinom(as.factor(Full_time_result_point) ~ pos_rating_regression + expect_point_H + streak_diff, data = train_data_month)
  multinom_full <- nnet::multinom(as.factor(Full_time_result_point) ~ pos_rating_regression + expect_point_H + streak_diff + xg_diff, data = train_data_month)
  random_forest_pos <- ranger(Full_time_result_num ~ expect_point_H + streak_diff + xg_diff + pos_rating_regression, data = train_data_month, probability = TRUE)
  init_logit_gam_full <- gam(list(Full_time_result_num ~ s(expect_point_H) + s(streak_diff) 
                              + s(xg_diff) + s(pos_rating_regression), 
                            ~s(expect_point_H) + s(streak_diff) 
                              + s(xg_diff) + s(pos_rating_regression)),
                      data = train_data_month,
                      family = multinom(K = 2))
  random_forest_base <- ranger(Full_time_result_num ~ expect_point_H, data = train_data_month, probability = TRUE)
  
  #brier score dataframe 
  brier_score_month <- data.frame(matrix(nrow = 1))
  actual_outcomes <- test_data_month$Full_time_result_point 
  predicted_probs <- as.data.frame(predict(expect_model, newdata = test_data_month, type = "probs"))
  predictions_cal <- predict(expect_model, newdata = test_data_month,type="probs")
predictions

  brier_score_month['multinomial base'] = data.frame(
    actual_outcomes,
    predicted_probs) %>% 
    mutate(actual_outcomes = factor(actual_outcomes)) %>% 
    brier_class(actual_outcomes, X0:X3) %>% 
  ...
  brier_score_month <-brier_score_month[-c(1)] 
  row.names(brier_score_month)[1] <- "brier score"
  
  predictions <- predict(init_logit_base_gam, newdata = test_data_month)

  #Predicting the grier score
  actual_outcomes_gams <- test_data_month$Full_time_result_point 
  predicted_probs_gams <- predict(init_logit_base_gam, newdata = test_data_month, type = "response")  

  brier_score_month['base GAMs'] = data.frame(
    actual_outcomes_gams,
    predicted_probs_gams) %>% 
    mutate(actual_outcomes_gams = factor(actual_outcomes_gams)) %>% 
    brier_class(actual_outcomes_gams, X1:X3) %>% 
  ...
  predictions <- predict(init_logit_gam, newdata = test_data_month)
  
  #Predicting the grier score
  actual_outcomes_gams <- test_data_month$Full_time_result_point 
  predicted_probs_gams <- predict(init_logit_gam, newdata = test_data_month, type = "response")  

  brier_score_month['streak GAMs'] = data.frame(
    actual_outcomes_gams,
    predicted_probs_gams
  ) %>% 
    mutate(actual_outcomes_gams = factor(actual_outcomes_gams)) %>% 
  ...
  predictions <- predict(init_logit_gam_xg, newdata = test_data_month)

  #Predicting the grier score
  actual_outcomes_gams <- test_data_month$Full_time_result_point 
  predicted_probs_gams <- predict(init_logit_gam_xg, newdata = test_data_month, type = "response")  

  brier_score_month['streak xg GAMs'] = data.frame(
    actual_outcomes_gams,
    predicted_probs_gams
  ) %>% 
    mutate(actual_outcomes_gams = factor(actual_outcomes_gams)) %>% 
  ...
  rf_predictions <- predict(random_forest, data = test_data_month)

  #Predicting the grier score
  rf_outcomes <- test_data_month$Full_time_result_point 
  rf_probs <- predict(random_forest, data = test_data_month, type = "response")  

  brier_score_month['rand forest streak Xg'] = data.frame(
    rf_outcomes,
    rf_probs
  ) %>% 
    mutate(rf_outcomes = factor(rf_outcomes)) %>% 
  ...
  #Predicting the grier score
  actual_outcomes <- test_data_month$Full_time_result_point 
  predicted_probs <- as.data.frame(predict(multinom_pos_reg, newdata = test_data_month, type = "probs"))
  base_data_frame <- cbind(actual_outcomes, predicted_probs)

  #Brier Score
  brier_score_month['multinomial pos'] = data.frame(
    actual_outcomes,
    predicted_probs
  ) %>% 
    mutate(actual_outcomes = factor(actual_outcomes)) %>% 
  ...
  #Predicting the grier score
  actual_outcomes_gams <- test_data_month$Full_time_result_point 
  predicted_probs_gams_pos <- predict(init_logit_gam_pos, newdata = test_data_month, type = "response")  
  gams_dataframe_predict <- data.frame(actual_outcomes_gams, predicted_probs_gams_pos)

  brier_score_month['Pos GAMs'] =  gams_dataframe_predict %>% 
    mutate(actual_outcomes_gams = factor(actual_outcomes_gams)) %>% 
    brier_class(actual_outcomes_gams, X1:X3) %>% 
    select(.estimate)

  actual_outcomes <- test_data_month$Full_time_result_point 
  predicted_probs <- as.data.frame(predict(multinom_betting_pos, newdata = test_data_month, type = "probs"))
  predictions_best <- predict(multinom_betting_pos, newdata = test_data_month,type="probs")
predictions

  #Brier Score
  brier_score_month['multinomial betting pos'] = data.frame(
    actual_outcomes,
    predicted_probs
  ) %>% 
    mutate(actual_outcomes = factor(actual_outcomes)) %>% 
  ...
  actual_outcomes <- test_data_month$Full_time_result_point 
  predicted_probs <- as.data.frame(predict(multinom_betting_pos_streak, newdata = test_data_month, type = "probs"))

  brier_score_month['multinomial betting pos streak'] = data.frame(
    actual_outcomes,
    predicted_probs
  ) %>% 
    mutate(actual_outcomes = factor(actual_outcomes)) %>% 
  ...
  actual_outcomes <- test_data_month$Full_time_result_point 
  predicted_probs <- as.data.frame(predict(multinom_full, newdata = test_data_month, type = "probs"))

  brier_score_month['multinomial full'] = data.frame(
    actual_outcomes,
    predicted_probs
  ) %>% 
    mutate(actual_outcomes = factor(actual_outcomes)) %>% 
  ...
  rfp_predictions <- predict(random_forest_pos, data = test_data_month)

  #Predicting the grier score
  rfp_outcomes <- test_data_month$Full_time_result_point 
  rfp_probs <- predict(random_forest_pos, data = test_data_month, type = "response")  

  brier_score_month['rand forest pos'] = data.frame(
    rfp_outcomes,
    rfp_probs
  ) %>% 
    mutate(rfp_outcomes = factor(rfp_outcomes)) %>% 
  ...
  actual_outcomes_gams <- test_data_month$Full_time_result_point 
  predicted_probs_gams <- predict(init_logit_gam_full, newdata = test_data_month, type = "response")  
  gams_dataframe_predict <- data.frame(actual_outcomes_gams, predicted_probs_gams)

  brier_score_month['Full GAMs'] =  gams_dataframe_predict %>% 
    mutate(actual_outcomes_gams = factor(actual_outcomes_gams)) %>% 
    brier_class(actual_outcomes_gams, X1:X3) %>% 
    select(.estimate)

  rf_outcomes <- test_data_month$Full_time_result_point 
  rf_probs_base <- predict(random_forest_base, data = test_data_month, type = "response")  
  rand_forest_predictions_base <- data.frame(rf_probs_base)

  brier_score_month['rand forest base'] = data.frame(
    rf_outcomes,
    rf_probs_base
  ) %>% 
    mutate(rf_outcomes = factor(rf_outcomes)) %>% 
  ...
  brier_score_month <- data.frame(t(brier_score_month)) 
  
  brier_month_vec <- as.vector(brier_score_month['brier.score'])
  brier_table_month <- cbind(brier_table_month, brier_month_vec)
  names(brier_table_month)[j] = paste(c(i, "month"), collapse = " ")
  j <- j+1
}

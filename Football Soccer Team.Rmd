...
plot_combined <- ggplot(calib_data, aes(x = x, y = y, color = group)) +
  ...
  ylim(0, 1) +
  scale_color_manual(values = c("red", "blue", "green"))  # Choose desired colors for the curves
...
bettingBasicPL2018 %>% 
  ggplot() +
  geom_histogram(aes(x = xg_diff), fill = "darkred", alpha = 0.4, bins = 18) +
  geom_histogram(aes(x = expect_point_diff_bet), fill = "blue", alpha = 0.4, bins = 18) +
  facet_wrap(~Full_time_result) +
  labs(x = "expected goals differential", color = "w/o betting odds") +
  theme_bw() 
```

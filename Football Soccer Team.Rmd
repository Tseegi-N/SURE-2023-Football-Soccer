...
accuracy_multinom <- sum(predicted_labels == true_labels) / length(true_labels)
...
rf_predicted_labels <- apply(predictions$predictions, 1, which.max) - 1
...
predicted_labels_gam <- max.col(predicted_probs_gams_base)
```
```{r}
#Accuracy and brier score table

model_names <- c("Random Forest", "Base gams model", "Gams Model xG", "Multinomial")  
accuracies <- c(accuracy_rf,accuracy_gams_base, accuracy_gams, accuracy_multinom)  

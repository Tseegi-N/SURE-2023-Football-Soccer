...
init_logit_gam_xg <- gam(as.factor(Full_time_result_point) ~ s(xg_diff),
                      data = filter(bettingBasicPL2018, is_train == 1),
                      family = binomial, method = "REML")
...
expect_model <- multinom(as.factor(Full_time_result_point) ~ expect_point_H, data = train_data)
summary(expect_model)
...
```{r}
predictions <- predict(init_logit_gam, newdata = test_data)
